...
| auto_module_px_size {: .kny-mono-font } | {{ kny:godot bool }}                                                 | Automatically set the module pixel size based on the size. Do not use expand mode `KEEP_SIZE` when using it.<br>Turn this off when the QR Code is resized often, as it impacts the performance quite heavily. |
...
{{ kny:godot Image }} generate_image(module_px_size: {{ kny:godot int }}=1, light_module_color: {{ kny:godot Color }}=Color.WHITE, dark_module_color: {{ kny:godot Color }}=Color.BLACK) {: .kny-mono-font }
