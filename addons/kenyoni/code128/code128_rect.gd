@tool
@icon("res://addons/kenyoni/code128/icon.svg")
extends TextureRect
class_name Code128Rect

const Code128 := preload("res://addons/kenyoni/code128/code128.gd")
const Code128Renderer := preload("res://addons/kenyoni/code128/renderer.gd")
...
var _renderer: Code128Renderer = Code128Renderer.new()

func set_number(new_number: String) -> void:
    new_number = Code128._number_rx.sub(new_number, "", true)
...
func update() -> void:
    ...
    if number.length() == 12:
        var checksum_digit: int = Code128.checksum(int(number))
        number += str(checksum_digit)
    self._ean13_value = number
    self._cached_data = Code128.encode(number)
    self._update_texture()
...
func _get_configuration_warnings() -> PackedStringArray:
    ...
    if number.length() == 13 && !Code128.validate(number):
...
func _update_module_size() -> void:
    self.module_size = Code128Renderer.calculate_module_size(self.size, self.add_quiet_zone)
